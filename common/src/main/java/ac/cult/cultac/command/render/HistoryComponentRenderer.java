...
public final class HistoryComponentRenderer {
    ...
    private static final String SESSION_ROW_FALLBACK = "<hover:show_text:'&bSession &7%session_uuid%"
            ...
            + "&8[&b%cult_version%&8] &8[&b%server_name%&8] &8[&b%client_version%&8]"
     ...
     * disambiguated {@code /cult history player <target>} form so a shared
    ...
    private static Component renderSummaryLine(Sender sender, ConfigManager cfg, SessionSummary s, boolean ongoing,
        ...
        RenderedTemplate rendered = parse(sender, cfg, "cult-history-session", SESSION_ROW_FALLBACK,
                Map.ofEntries(
                        ...
                        Map.entry("cult_version", TemplateValue.raw(nullToUnknown(s.grimVersion()))),
    ...
    public static @NotNull List<Component> renderSessionDetail(
        ...
        Map<String, TemplateValue> metaVars = Map.ofEntries(
                ...
                Map.entry("cult_version", TemplateValue.raw(nullToUnknown(d.grimVersion()))),
     ...
     *
     * <p>Used by {@code /cult history --name <regex>} / {@code --match
