...
public final class PacketHandlerScanner {
    ...
    private static List<Method> annotatedMethods(Class<?> listenerClass) {
        ...
        for (Class<?> current = listenerClass; current != null && current != Object.class; current = current.getSuperclass()) {
            ...
            for (Method method : declaredMethods) {
                ...
                if (method.isAnnotationPresent(GrimPacketHandler.class)) {
                    methods.add(method);
                }
            }
        }
    ...
    private static HandlerSignature signature(Method method, Class<?> listenerClass) {
        ...
        if (parameterTypes.length != 3) {
            throw invalid(listenerClass, method, "must accept exactly event, GrimPlayer, and one concrete packet");
        }
        if (parameterTypes[1] != GrimPlayer.class) {
            throw invalid(listenerClass, method, "second parameter must be GrimPlayer");
        }
    ...
    private static List<Class<? extends Packet<?>>> packetTypes(
        ...
        GrimPacketHandler handlerAnnotation = method.getAnnotation(GrimPacketHandler.class);
        ...
        GrimPacketGroup groupAnnotation = method.getAnnotation(GrimPacketGroup.class);
    ...
    private static IllegalStateException invalid(Class<?> listenerClass, Method method, String reason, Throwable cause) {
        return new IllegalStateException("Invalid @GrimPacketHandler in "
