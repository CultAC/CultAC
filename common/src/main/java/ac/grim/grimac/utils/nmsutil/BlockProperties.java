...
public class BlockProperties {
     ...
     */
    public static Material getOnPos(GrimPlayer player, MainSupportingBlockData mainSupportingBlockData, Vec3 playerPos) {
        BlockPos pos = getOnPos(player, playerPos, mainSupportingBlockData, 0.2F);
        return player.compensatedWorld.getMaterialAt(pos);
    }

    public static float getFriction(GrimPlayer player, MainSupportingBlockData mainSupportingBlockData, Vec3 playerPos) {
    ...
    public static float getBlockSpeedFactor(GrimPlayer player, MainSupportingBlockData mainSupportingBlockData, Vec3 playerPos) {
        if (player.isGliding || player.isFlying) return 1.0f;

        BlockState inBlock = player.compensatedWorld.getBlockStateAt(GrimMath.floor(playerPos.x), GrimMath.floor(playerPos.y), GrimMath.floor(playerPos.z));
    ...
    public static boolean onHoneyBlock(GrimPlayer player, MainSupportingBlockData mainSupportingBlockData, Vec3 playerPos) {
        Material inBlock = player.compensatedWorld.getBlockStateAt(GrimMath.floor(playerPos.x), GrimMath.floor(playerPos.y), GrimMath.floor(playerPos.z)).getBukkitMaterial();
     ...
     */
    private static Material getBlockPosBelowThatAffectsMyMovement(GrimPlayer player, MainSupportingBlockData mainSupportingBlockData, Vec3 playerPos) {
        BlockPos pos = getOnPos(player, playerPos, mainSupportingBlockData, 0.500001F);
        return player.compensatedWorld.getMaterialAt(pos);
    }

    private static BlockPos getOnPos(GrimPlayer player, Vec3 playerPos, MainSupportingBlockData mainSupportingBlockData, float searchBelowPlayer) {
        ...
        if (mainBlockPos != null) {
            ...
            return shouldReturn ? mainBlockPos.atY(GrimMath.floor(playerPos.y - (double) searchBelowPlayer)) : mainBlockPos;
        }

        return new BlockPos(GrimMath.floor(playerPos.x), GrimMath.floor(playerPos.y - searchBelowPlayer), GrimMath.floor(playerPos.z));
    }
