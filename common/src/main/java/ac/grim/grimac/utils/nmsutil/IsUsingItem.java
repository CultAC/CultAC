...
public class IsUsingItem {
    ...
    public static boolean isUsingItem(GrimPlayer player) {
    ...
    public static boolean isSlowDueToUsingItem(GrimPlayer player) {
    ...
    public static float getUseItemSpeedMultiplier(GrimPlayer player) {
        ItemStack activeItem = getActiveItem(player);
        if (activeItem == null || activeItem.getType().isAir()) {
            return 1.0F;
        }
    ...
    public static void stopUseItem(GrimPlayer player) {
        if (player.bukkitPlayer == null) return;
        player.bukkitPlayer.clearActiveItem();
    }

    private static ItemStack getActiveItem(GrimPlayer player) {
