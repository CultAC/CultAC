...
public final class NativeBlockCollisionHelper {
    ...
    public static CollisionBox getCollisionBox(GrimPlayer player, BlockState state, int x, int y, int z) {
    ...
    public static CollisionBox getCollisionBox(GrimPlayer player, BlockData data, int x, int y, int z) {
    ...
    public static VoxelShape getSelectionShape(GrimPlayer player, BlockState state, int x, int y, int z) {
        if (player == null || player.compensatedWorld == null || state == null || state.isAir()) {
            return net.minecraft.world.phys.shapes.Shapes.empty();
        }
    ...
    public static CollisionContext collisionContext(GrimPlayer player) {
        return collisionContext(player, player == null ? Double.NaN : player.y);
    }

    public static CollisionContext collisionContext(GrimPlayer player, double entityBottom) {
        if (player == null) {
            return CollisionContext.empty();
        }
        return new GrimCollisionContext(player, entityBottom);
    }

    private static final class GrimCollisionContext implements CollisionContext {
        private final boolean descending;
        private final double entityBottom;
        private final net.minecraft.world.item.ItemStack heldItem;

        private GrimCollisionContext(GrimPlayer player, double entityBottom) {
            this.descending = player.isSneaking;
            this.entityBottom = entityBottom;
            this.heldItem = heldItem(player);
        }

        private static net.minecraft.world.item.ItemStack heldItem(GrimPlayer player) {
            try {
