...
public class BoatTransform implements UncertaintyHandler{
    ...
    public PredVector handleUncertainty(GrimPlayer player, ValidMovements valid, PredictionResult result, SimulationContext context, PredictionResult lastResult, PredVector start, Vec3 end) {
        if (context.getVehicle() == null || !ac.grim.grimac.utils.nmsutil.EntityTypeUtil.isBoat(context.getVehicle().type)) return start;
    ...
    public static void commitAcceptedBoatTickState(GrimPlayer player, PredictionResult result) {
        SimulationContext context = result.getSimulationContext();
        if (context.getVehicle() == null || !ac.grim.grimac.utils.nmsutil.EntityTypeUtil.isBoat(context.getVehicle().type)) return;
    ...
    public static String debugBoatTick(GrimPlayer player, SimulationContext context, Vec3 vector) {
        if (context.getVehicle() == null || !ac.grim.grimac.utils.nmsutil.EntityTypeUtil.isBoat(context.getVehicle().type)) {
            return null;
        }
    ...
    public static boolean usesProvenWaterEntryPositionSnap(GrimPlayer player, SimulationContext context) {
        if (context.getVehicle() == null || !ac.grim.grimac.utils.nmsutil.EntityTypeUtil.isBoat(context.getVehicle().type)) {
            return false;
        }
    ...
    public static Vec3 applyBoatBaseTickWaterCurrent(GrimPlayer player, PacketEntity vehicle, Vec3 physicalPosition, Vec3 vector) {
    ...
    private static Vec3 applyBoatWaterCurrent(GrimPlayer player, SimulationContext context, Vec3 vector) {
    ...
    private static Vec3 applyBoatBaseTickWaterCurrent(GrimPlayer player, SimpleCollisionBox fluidBox, Vec3 vector) {
    ...
    private static FloatBoatResult floatBoat(GrimPlayer player, SimulationContext context, Vec3 vector, double lastYd, BoatTickState state) {
        SimpleCollisionBox boatBox = context.getFromMaximumExtent();
    ...
    private static BoatTickResult selectBoatTick(GrimPlayer player, SimulationContext context, Vec3 vector, BoatTickState state, Vec3 target) {
    ...
    private static BoatTickResult computeBoatTick(GrimPlayer player, SimulationContext context, Vec3 vector, BoatTickState state) {
    ...
    private static boolean waterEntrySnapHasNoCollision(GrimPlayer player, SimulationContext context, SimpleCollisionBox movedBox) {
        return findFirstWaterEntryIntersection(player, context, movedBox) == null;
    }

    private static String debugWaterEntryProbe(GrimPlayer player, SimulationContext context, BoatTickState state) {
        if (state.oldStatus() != BoatEntityStatus.IN_AIR
                || state.status() == BoatEntityStatus.IN_AIR
                || state.status() == BoatEntityStatus.ON_LAND) {
            return "n/a";
    ...
    private static SimpleCollisionBox findFirstWaterEntryIntersection(GrimPlayer player, SimulationContext context, SimpleCollisionBox movedBox) {
    ...
    public static float getWaterLevelAbove(GrimPlayer player, SimpleCollisionBox axisalignedbb, double lastYd) {
    ...
    private static BoatControlResult controlBoat(GrimPlayer player, SimulationContext context, Vec3 vector, BoatTickState state) {
    ...
    private record BoatTickState(BoatEntityStatus oldStatus, BoatEntityStatus status, double waterLevel, float landFriction,
        ...
        private static BoatTickState capture(GrimPlayer player) {
        ...
        private void apply(GrimPlayer player) {
