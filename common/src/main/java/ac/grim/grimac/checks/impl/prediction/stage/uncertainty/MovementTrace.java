...
public record MovementTrace(PredVector position, Vec3 positionOnlyDelta, Vec3 collisionBaseOffset,
                            Vec3 preCollisionMovement) {
    public static MovementTrace start(PredVector position) {
        return new MovementTrace(position, Vec3.ZERO, Vec3.ZERO, position);
    }

    public MovementTrace withPosition(PredVector position) {
        return new MovementTrace(position, positionOnlyDelta, collisionBaseOffset, preCollisionMovement);
    }

    public MovementTrace withPreCollisionMovement(Vec3 movement) {
        return new MovementTrace(position, positionOnlyDelta, collisionBaseOffset, movement);
    }
    ...
    public MovementTrace addPositionOnlyDelta(Vec3 delta) {
        ...
        return new MovementTrace(position, positionOnlyDelta.add(delta), collisionBaseOffset, preCollisionMovement);
    }
    ...
    public MovementTrace addCollisionBaseOffset(Vec3 delta) {
        ...
        return new MovementTrace(position, positionOnlyDelta, collisionBaseOffset.add(delta), preCollisionMovement);
    }
}
