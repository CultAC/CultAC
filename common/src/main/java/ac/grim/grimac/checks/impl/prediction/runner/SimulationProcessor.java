...
import ac.grim.grimac.checks.impl.prediction.PredictionResult.Flag;
import ac.grim.grimac.checks.impl.prediction.checks.PredCheckRunner;
...
import ac.grim.grimac.network.packet.NmsPacketUtil.MovePlayerData;
...
import ac.grim.grimac.utils.data.StuckSpeedData;
...
public class SimulationProcessor extends GrimProcessor implements PositionListener, ClientTickEndListener {
   ...
   public SimulationProcessor(GrimPlayer playerData) {
      super(playerData);
   }
   ...
   private boolean changedBlockCanApplyPowderSnowStuckSpeed(SimulationContext context, BlockPos pos, BlockData to) {
      ...
      BlockPos rootBlockPosition = new BlockPos(GrimMath.floor(context.getEnd().x), GrimMath.floor(context.getEnd().y), GrimMath.floor(context.getEnd().z));
