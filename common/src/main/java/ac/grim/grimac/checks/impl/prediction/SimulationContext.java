...
import ac.grim.grimac.bedrock.protocol.BedrockProtocolVersion;
...
public class SimulationContext {
    ...
    public float getMaxSpeed(GrimPlayer player) {
    ...
    public float getGroundAirSpeed(GrimPlayer player) {
        float blockUnderPlayer = BlockProperties.getFriction(player, lastTickMainSupportingBlockData, start);
        float maxSpeed = 0;
    ...
    public float getFluidSpeed(GrimPlayer player, boolean water, boolean lava) {
        float blockUnderPlayer = BlockProperties.getFriction(player, lastTickMainSupportingBlockData, start);
        return getFrictionInfluencedSpeed(blockUnderPlayer, player, false, water, lava);
    }

    public float getMoveRelativeSpeed(GrimPlayer player, boolean onGround, boolean water, boolean lava) {
        float blockUnderPlayer = BlockProperties.getFriction(player, lastTickMainSupportingBlockData, start);
    ...
    private float getFrictionInfluencedSpeed(float friction, GrimPlayer player) {
        float maxSpeed = 0;
        for (boolean onGround : worldData.getLastOnGround().getStates()) {
    ...
    private double getPlayerMovementSpeed(GrimPlayer player) { if (vehicle == null) {
    ...
    private boolean isMountedStriderSuffocating(GrimPlayer player) {
