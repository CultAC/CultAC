...
public class SuperDebug extends GrimProcessor implements PostPredictionListener {
    ...
    public void onPredictionComplete(PredictionComplete predictionComplete) {
        ...
        int minX = GrimMath.floor(player.boundingBox.minX) - 2;
        int maxX = GrimMath.ceil(player.boundingBox.maxX) + 2;
        int minY = GrimMath.floor(player.boundingBox.minY) - 2;
        int maxY = GrimMath.ceil(player.boundingBox.maxY) + 2;
        int minZ = GrimMath.floor(player.boundingBox.minZ) - 2;
        int maxZ = GrimMath.ceil(player.boundingBox.maxZ) + 2;
        ...
        for (int y = minY; y <= maxY; y++) {
            for (int z = minZ; z <= maxZ; z++) {
                for (int x = minX; x <= maxX; x++) {
                    String text = ac.grim.grimac.network.protocol.util.SpigotConversionUtil.fromNmsBlockState(
    ...
    private List<Vec3> debugRideableInputVectors(SimulationContext context, PacketEntity vehicle) {
        if (!player.packetStateData.isVehicleMovementFromClientTick()
                ...
                || ac.grim.grimac.utils.nmsutil.EntityTypeUtil.isBoat(vehicle.type)) {
            return List.of();
        }
