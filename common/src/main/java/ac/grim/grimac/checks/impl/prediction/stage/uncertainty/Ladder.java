...
public class Ladder implements UncertaintyHandler {
    ...
    public PredVector handleUncertainty(GrimPlayer player, ValidMovements valid, PredictionResult result, SimulationContext context, PredictionResult lastResult, PredVector start, Vec3 end) {
        ...
        double clampedX = GrimMath.clamp(start.x, -0.15F, 0.15F);
        double clampedZ = GrimMath.clamp(start.z, -0.15F, 0.15F);
        ...
        if (context.getWorldData().getClimbingAtStart().isDesync() || context.getWorldData().maybeInLiquid()) {
            clampedX = GrimMath.clamp(end.x, Math.min(start.x, clampedX), Math.max(start.x, clampedX));
            clampedY = GrimMath.clamp(end.y, Math.min(start.y, clampedY), Math.max(start.y, clampedY));
            clampedZ = GrimMath.clamp(end.z, Math.min(start.z, clampedZ), Math.max(start.z, clampedZ));
        }
