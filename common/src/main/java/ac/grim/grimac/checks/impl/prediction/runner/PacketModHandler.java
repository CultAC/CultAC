...
public class PacketModHandler extends Check implements EngineCheck, PostPredictionListener {
    ...
    public PacketModHandler(GrimPlayer grimPlayer, CheckInfo info) { super(grimPlayer, info); }
    ...
    protected GrimPlayer.TrackedTransaction bundlePacketWithTrailingTransaction(PacketSendEvent event, Packet<? super ClientGamePacketListener> packet) {
    ...
    protected int handleEvent(Vec3 playerVelocity, boolean isVel, PacketSendEvent event, int sourceEntityId) {
        // Wrap velocity between two transactions, matching Grim3.0-clean's
    ...
    public int handleDebugDirectPacketEvent(
        ...
        GrimPlayer.TrackedTransaction transaction = shouldUseBundledProof()
        ...
        GrimPlayer.TrackedTransaction firstProof = sendDirectTrackedTransaction();
        if (firstProof == null) {
    ...
    private GrimPlayer.TrackedTransaction sendDirectTrackedTransaction() {
    ...
    private boolean shouldCarryPreviousPacketModifier(TransactionVel previous, boolean currentIsVelocity, int currentTransaction) {
        ...
        // ServerboundClientTickEndPacket. Once Grim has seen that tick-end after the
