...
final class SmoketestSnapshotBridge {
    private static final String CHANNEL = "grim:smoketest_snapshot";
    ...
    static boolean handle(GrimPlayer player, String channel, byte[] data) {
        ...
        try {
            ...
            LogUtil.warn("Grim smoketest snapshot failed player=" + player.getName()
    ...
    private static void compare(GrimPlayer player, Snapshot snapshot) {
        ...
        for (BlockSnapshot block : snapshot.blocks()) {
            ...
            int grimStateId = Block.getId(compensatedState);
            String grimState = String.valueOf(compensatedState);
            ...
            if (!block.state().equals(grimState)) {
        ...
        if (snapshot.selectedSlot() >= 0 && player.packetStateData.lastSlotSelected != snapshot.selectedSlot()) {
            mismatches.add("selected-slot expected=" + snapshot.selectedSlot()
                    + " grim=" + player.packetStateData.lastSlotSelected);
        }
        ...
        if (mismatches.isEmpty()) {
            ...
            LogUtil.info("Grim smoketest snapshot passed player=" + player.getName()
                    + " tick=" + snapshot.tick()
    ...
    private static void compareItem(List<String> mismatches, String owner, ItemSnapshot expected, org.bukkit.inventory.ItemStack item) {
        net.minecraft.world.item.ItemStack grim = SpigotConversionUtil.toNmsItemStack(item);
        String key = grim.isEmpty() ? "minecraft:air" : NmsIdentifierUtil.registryKey(BuiltInRegistries.ITEM, grim.getItem());
        int count = grim.isEmpty() ? 0 : grim.getCount();
        String components = grim.isEmpty() ? "" : String.valueOf(grim.getComponentsPatch());
