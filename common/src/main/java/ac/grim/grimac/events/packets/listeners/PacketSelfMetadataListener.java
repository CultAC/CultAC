...
public class PacketSelfMetadataListener {
    ...
    private void handleSetEntityData(PacketSendEvent event, GrimPlayer player, ClientboundSetEntityDataPacket entityMetadataPacket) {
        ...
        if (entityMetadataPacket.id() == player.entityID) {
            ...
            boolean changed = false;
                ...
                if (bedObject != null) { Optional<BlockPos> bed = (Optional<BlockPos>) bedObject.value();
                    ...
                    // (BedrockMovePlayer.java:74-79). Reuse Grim's existing
    ...
    private void handleAnimate(PacketSendEvent event, GrimPlayer player, ClientboundAnimatePacket animation) {
