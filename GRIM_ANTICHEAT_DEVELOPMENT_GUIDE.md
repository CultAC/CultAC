# Grim Anticheat Development Guide

This guide describes how to write Grim checks and compensation code in a way that matches vanilla movement, vanilla protocol timing, and latency. Grim should not be a collection of exemptions. Grim should be a latency-compensated reverse simulation of what the vanilla client could have done.

## Philosophy

Grim's job is to reconstruct the client's legal movement envelope.

For every player movement packet, Grim should answer:
...
Grim movement code should mirror the vanilla client pipeline.
...
Treat the tick-end packet as the delimiter that clears per-client-tick state. Packets after tick-end belong to the next client tick even if no Grim transaction has been acknowledged yet.
...
- This is separate from keepalive. Keepalive has its own packet pair and can be delayed by client conditions such as frozen polling. Ping/pong is the better Grim transaction primitive.

Grim currently uses `ClientboundPingPacket` as the transaction packet and only accepts `ServerboundPongPacket` ids it sent. Keep this property:

- only Grim-generated ping ids should advance Grim transaction state
- third-party or server-generated pings must not corrupt Grim's transaction order
...
Do not use keepalive as the primary movement compensation barrier when ping/pong is available. It does not map as cleanly to a deliberately placed Grim transaction boundary.
...
For Grim, a bundle is a causality group.
...
Future Grim bundle support should track:
...
Do not ignore bundle delimiters if Grim's Netty hook sees them. Do not flatten bundles in a way that loses the "these packets arrived together before the next packet" property.

## Latency Model

Latency is not a boolean exempt state.

Latency means the server and Grim know about a packet before the client necessarily knows about it. Grim must preserve both timelines:
...
1. Observed by Grim on outbound packet send.
...
- copy it into Grim-owned data structures
- synchronize that snapshot into the Netty timeline using transactions
- read the Grim-owned compensated state from packet/check code

Allowed packet-thread data sources:

- packet contents
- Grim-owned compensated world/entity/inventory state
...
Grim's packet-time state should live on the player's Netty/event-loop timeline.

Use `GrimPlayer.runSafely()` or `ChannelHelper.runInEventLoop()` when a mutation must occur on the player's Netty event loop. Use transaction-scheduled tasks when client receipt matters. Use Bukkit/Folia scheduling only to capture server-thread state, then copy it back into Grim-owned state safely.
...
- the remote player exists in Grim's entity map
...
Modern Grim should make `ServerboundClientTickEndPacket` first-class.
...
The client tick boundary and clientbound receipt boundary are different concepts. Grim should model both.

## Ping/Pong Implementation Direction

Grim should use ping/pong as a precise transaction layer.
...
- never advance transaction state from pongs Grim did not send
...
- do not send a Grim transaction in the middle of a bundle unless it is actually written there on the wire
...
Before merging a Grim anticheat change, answer:
...
- Does it handle ping/pong pongs that Grim did not send?
