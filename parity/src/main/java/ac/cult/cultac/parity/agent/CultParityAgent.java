 ...
 *
 * <p>The transformed runtime only references {@link CultParityBootstrap}.
 * That class is copied into a tiny bootstrap jar so the instrumented Cult
 ...
 */
public final class CultParityAgent {
    private static final String CHECK_BASE = "ac/cult/cultac/checks/Check";
    private static final String BOOTSTRAP_CLASS =
            "ac/cult/cultac/parity/agent/CultParityBootstrap";
    ...
    public static void premain(String arguments, Instrumentation instrumentation) throws Exception {
        ...
        CultParityBootstrap.agentStarted();
        ...
        if (arguments != null && !arguments.isBlank()) {
            for (String token : arguments.split(";")) {
                ...
                if (!key.isEmpty() && !value.isEmpty()) {
                    System.setProperty("cult.parity." + key, value);
                }
            }
        }
        copyEnvironment("CULT_PARITY_INVENTORY", "cult.parity.inventory");
        copyEnvironment("CULT_PARITY_TRACE", "cult.parity.trace");
        copyEnvironment("CULT_PARITY_CHECK", "cult.parity.traceStableKey");
        copyEnvironment("CULT_PARITY_OPTIONAL_STUBS", "cult.parity.optionalStubs");
    }
    ...
    private static Path createBootstrapJar() throws IOException {
        Path jar = Files.createTempFile("cult-parity-bootstrap-", ".jar");
        jar.toFile().deleteOnExit();
        List<String> entries = new ArrayList<>(List.of(BOOTSTRAP_CLASS + ".class"));
        if (Boolean.getBoolean("cult.parity.optionalStubs")) {
            entries.add("com/viaversion/viaversion/api/protocol/packet/PacketTracker.class");
        }
        try (JarOutputStream output = new JarOutputStream(Files.newOutputStream(jar))) {
            for (String entryName : entries) {
                try (InputStream input = CultParityAgent.class.getClassLoader().getResourceAsStream(entryName)) {
                    if (input == null) {
    ...
    private static final class CheckTransformer implements ClassFileTransformer {
        ...
        public byte[] transform(
            ...
            try {
                ...
                boolean manager = name.equals("ac/cult/cultac/manager/CheckManager")
                        || name.equals("ac/cult/cultac/manager/player/CheckManager")
                        || name.equals("ac/grim/grimac/manager/CheckManager")
                        || name.equals("ac/grim/grimac/manager/player/CheckManager");
                if (!manager && !name.startsWith("ac/cult/cultac/") && !name.startsWith("ac/grim/grimac/")) {
                    return null;
                }
                ...
                String classFilter = System.getProperty("cult.parity.classFilter", "").trim();
                if (!classFilter.isEmpty() && !matchesClassFilter(name, classFilter)) {
                    return null;
                }
                CultParityBootstrap.transformObserved(name, manager ? "manager" : "check");
                if ("false".equalsIgnoreCase(System.getProperty("cult.parity.transform", "true"))) {
                    return null;
                }
                String traceMode = System.getProperty("cult.parity.methodTrace", "full").trim().toLowerCase();
                ...
                reader.accept(new ClassVisitor(Opcodes.ASM9, writer) {
                    ...
                    public MethodVisitor visitMethod(
                        ...
                        if (check && method.equals("<init>") && isCheckBase(name)) {
                        ...
                        if (!traceMethods || !check
                                || (isCheckBase(name) && !BASE_TRACE_METHODS.contains(method))
                ...
                CultParityBootstrap.transformObserved(name, manager ? "complete-manager" : "complete-check");
                return transformed;
            } catch (Throwable failure) {
                CultParityBootstrap.transformObserved(
                        name,
        ...
        // The pinned baseline predates the CultAC package rename.
        private static boolean isCheckBase(String name) {
            return CHECK_BASE.equals(name) || "ac/grim/grimac/checks/Check".equals(name);
        }
