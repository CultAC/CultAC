...
`cultParity` is the fail-closed inventory, static-dependency, and live-trace
comparison harness for the runtime-registered Cult checks.

Run the complete driver with:

```text
scripts/run-cult-parity.sh --baseline-ref 73a835d04f9bfbc76dc1d41c24ea73f3aec2280c --checks all --artifact-root PATH
```
...
`cultParityLive` task in the pinned MCP/Paper smoketest workspace.
