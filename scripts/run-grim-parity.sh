...
if [[ "$checks" != "all" ]]; then
    echo "grimParity requires --checks all for final execution" >&2
    exit 2
fi
...
if [[ "$baseline_ref" != "$audited_baseline" ]]; then
    echo "grimParity baseline is locked to audited commit $audited_baseline" >&2
    exit 2
fi

if ! resolved_baseline="$(git -C "$repo_root" rev-parse --verify "${baseline_ref}^{commit}" 2>/dev/null)"; then
    echo "audited grimParity baseline commit is unavailable: $baseline_ref" >&2
    exit 2
fi
if [[ "$resolved_baseline" != "$audited_baseline" ]]; then
    echo "grimParity baseline resolved to $resolved_baseline, expected $audited_baseline" >&2
    exit 2
fi
...
baseline_parent="$(mktemp -d "${TMPDIR:-/tmp}/grim-parity-worktree.XXXXXX")"
...
(
    cd "$repo_root"
    ./gradlew :parity:grimParityAgentJar --no-daemon --console=plain
) >"$artifact_root/build-logs/agent-build.log" 2>&1
agent="$repo_root/parity/build/libs/grim-parity-agent.jar"
test -f "$agent"
...
run_inventory_probe() {
    ...
    mkdir -p "$probe_root"
    GRIM_PARITY_MODE=live \
    GRIM_PARITY_SIDE="$side" \
    GRIM_PARITY_CHECK=grim.parity.inventory \
    GRIM_PARITY_PATH=inventory \
    GRIM_PARITY_PROFILE=zero-latency \
    GRIM_PARITY_SEED=9001 \
    GRIM_PARITY_JAR="$jar" \
    GRIM_PARITY_ARTIFACT_DIR="$probe_root" \
    GRIM_PARITY_ACTION_TAPE=spawn-stationary \
    GRIM_PARITY_VALIDITY_GATE='scenario-passed;scenario-valid' \
    GRIM_PARITY_EXPECTED_OUTCOME=inventory \
    GRIM_PARITY_NETWORK_SCHEDULE='[{"direction":"inbound","baseMillis":0,"jitterMillis":0},{"direction":"outbound","baseMillis":0,"jitterMillis":0}]' \
    GRIM_PARITY_PORT="$port" \
    GRIM_PARITY_AGENT="$agent" \
    GRIM_PARITY_DISABLE_SAFETY_AGENT=true \
    "$repo_root/scripts/run-grim-parity-live.sh" \
...
(
    cd "$repo_root"
    ./gradlew :parity:grimParity --no-daemon --console=plain \
        -PgrimParity.checks="$checks" \
        -PgrimParity.baselineRef="$baseline_ref" \
        -PgrimParity.baselineCommit="$baseline_commit" \
        -PgrimParity.currentCommit="$current_commit" \
        -PgrimParity.artifactRoot="$artifact_root" \
        -PgrimParity.baselineJar="$artifact_root/jars/baseline.jar" \
        -PgrimParity.currentJar="$artifact_root/jars/current.jar" \
        -PgrimParity.baselineSource="$baseline_worktree/common/src/main/java" \
        -PgrimParity.currentSource="$repo_root/common/src/main/java" \
        -PgrimParity.baselineInventory="$artifact_root/inventory/baseline.jsonl" \
        -PgrimParity.currentInventory="$artifact_root/inventory/current.jsonl" \
        -PgrimParity.scenarioCatalog="$repo_root/parity/scenarios/shared-check-scenarios.json" \
        -PgrimParity.mappingFile="$repo_root/parity/config/check-equivalence-mappings.tsv" \
        -PgrimParity.classificationFile="$repo_root/parity/config/check-classifications.tsv" \
        -PgrimParity.repositoryRoot="$repo_root" \
        -PgrimParity.liveCommand="$repo_root/scripts/run-grim-parity-live.sh"
